...
class RemoteWebDashboard {
 ...
 private:
  ...
  String renderDashboard() const {
    ...
    html += F(".shell{max-width:1180px;margin:0 auto}.top{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center;margin-bottom:12px}");
    html += F("h1{font-size:28px;line-height:1.05;margin:0}");
    html += F(".status{border:1px solid #b7c8cd;background:#fff;padding:10px 12px;border-radius:8px;min-width:240px}");
    html += F(".status strong{display:block;font-size:12px;text-transform:uppercase;letter-spacing:.04em;color:#5b6e75}.status p{margin:4px 0 0}");
    ...
    html += F("@media (max-width:640px){body{padding:10px}.top{grid-template-columns:1fr}.commands{grid-template-columns:1fr}.status{min-width:0}.controls{top:0}}");
    html += F("@media (prefers-color-scheme:dark){:root,body{background:#101415;color:#eef4f5}.controls{background:#101415;border-color:#263237}.status,.profile-card,.empty{background:#182023;border-color:#334247}.search{background:#182023;color:#eef4f5;border-color:#46575d}.meta,.tag{color:#a9b8bc}button{background:#243034;color:#eef4f5;border-color:#46575d}button:hover{background:#314146}.pill{background:#26373b;color:#d7e8eb;border-color:#46575d}.sweep button{background:#3a2d18;border-color:#80652f}}");
