...
class RemoteWebDashboard {
 public:
  RemoteWebDashboard(ir_sender::UniversalIrSender& sender,
                     ir_receiver::IntegratedIrReceiver& receiver)
      : sender_(sender),
        receiver_(receiver),
 ...
 private:
  ...
  void handleSend() {
    if (receiver_.mode() != ir_receiver::OperationMode::Send) {
      lastStatus_ = F("Einlese-Modus aktiv");
      server_.send(409, "text/plain",
                   "Bitte erst in den Senden-Modus wechseln.");
      return;
    }
  ...
  void handleMode() {
    if (!server_.hasArg("mode")) {
      server_.send(400, "text/plain", "mode fehlt");
      return;
    }

    const String mode = server_.arg("mode");
    if (mode == F("capture")) {
      receiver_.setMode(ir_receiver::OperationMode::Capture);
      lastStatus_ = F("Einlesen aktiv");
      sendCaptureStatusJson();
      return;
    }

    if (mode == F("send")) {
      receiver_.setMode(ir_receiver::OperationMode::Send);
      lastStatus_ = F("Senden aktiv");
      sendCaptureStatusJson();
      return;
    }

    server_.send(400, "text/plain", "ungueltiger mode");
  }
  ...
  void appendModePanel(String& html) const {
  ...
  void appendDashboardScript(String& html) const {
    html += F("<script>");
    html += F("var activeCategory='';var activeProfile='';var activeMode='send';");
    html += F("function normalize(v){return (v||'').toLowerCase();}");
    html += F("function setStatus(text){var status=document.getElementById('statusText');if(status){status.textContent=text;}}");
  ...
  static void appendJsonEscaped(String& json, const char* text) {
    for (const char* c = text; *c != '\0'; ++c) {
      switch (*c) {
        case '\\':
          json += F("\\\\");
  ...
  ir_receiver::IntegratedIrReceiver& receiver_;
  WebServer server_;
  String lastStatus_;
};
